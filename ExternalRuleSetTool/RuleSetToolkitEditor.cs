...
namespace WuKeSong.Tool
{
    public partial class RuleSetToolkitEditor : Form
    {
        ...
        private List<RuleSetData> GetRuleSets()
        {
            ...
            {
                sqlConn.Open();
                string commandString = "SELECT Name, Version, Version, RuleSet, Status, AssemblyPath, ActivityName, TimeStamp FROM TriggerRule ORDER BY Name,Status";
                ...
                while (reader.Read())
                {
                    try
                    {
                        ...
                        data.MajorVersion = int.Parse(reader.GetString(1));
                        data.OriginalMajorVersion = data.MajorVersion;
                        data.MinorVersion = int.Parse(reader.GetString(2));
                ...
                using (SqlConnection sqlConn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlTransaction transaction = sqlConn.BeginTransaction())
                    {
                        ...
                        foreach (RuleSetData data in deletedRuleSetDataCollection)
                        {
                            ...
                            {
                                deleteCommand.CommandText = (string.Format(CultureInfo.InvariantCulture, "DELETE TriggerRule WHERE Name = @name AND Version = {0}", data.OriginalMajorVersion));
                        ...
                        foreach (RuleSetData data in ruleSetDataDictionary.Values)
                        {
                            if (data.Dirty == true)
                            {
                                ...
                                try
                                {
                                    if (!string.IsNullOrEmpty(data.OriginalName)) //delete the ruleset if it exists in the DB; OriginalName will be null if it was created in this session
                                    {
                                        deleteCommand.CommandText = (string.Format(CultureInfo.InvariantCulture, "DELETE TriggerRule WHERE Name = @name AND MajorVersion = {0}", data.OriginalMajorVersion));
                                        deleteCommand.Parameters["@name"].Value = data.OriginalName;
                                    ...
                                    using (SqlCommand insertCommand = new SqlCommand())
                                    {
                                        ...
                                        insertCommand.CommandText = (string.Format(CultureInfo.InvariantCulture, "INSERT TriggerRule (Name, Version, Status, AssemblyPath, ActivityName, TimeStamp,RuleSet ) VALUES('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", data.Name, data.MajorVersion.ToString(), "effective", data.AssemblyPath, data.ActivityName, DateTime.Now, data.RuleSetDefinition));
